...
;(function() {
    ...
    var dbDir = "waterQdb"; //離線資料存放路徑
    var illegalImgDir = "illegalImg"; //違規照片存放路徑
                        ...
                        if (destinationType == Camera.DestinationType.FILE_URI) {
                            if (targetHeight > 0 && targetWidth > 0) {
                                ...
                                Windows.Storage.StorageFolder.getFolderFromPathAsync(packageId.path).done(function (storageFolder) {
                                    ...
                                    /*20131113  marked
                                    illegalImgDate = new Date();
                                    Windows.Storage.StorageFolder.getFolderFromPathAsync(Windows.Storage.ApplicationData.current.localFolder.path + "\\" + dbDir + "\\" + illegalImgDir + "\\").done(
                                        function (dir) {
                                            picture.copyAsync(dir, illegalImgDate.getTime()+illegalImgFormat, Windows.Storage.NameCollisionOption.replaceExisting).then(function (storageFile) {
                                        var _imageUrl = URL.createObjectURL(storageFile);
                                        successCallback(_imageUrl);
                                    }, function () {
                                        fail(FileError.INVALID_MODIFICATION_ERR);
